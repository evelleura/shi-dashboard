...
| 2026-06-19 | FIX | 17 | "SPI 1 semua" di riwayat: EVM klasik memaksa SPI->1 saat proyek selesai (EV=PV=100% pada end_date). Fix: proyek `completed` pakai SPI = durasi_rencana/durasi_aktual (selesai cepat>1, telat<1); aktual = MAX(done status_changed_at)-start. Seed `SCHED_FACTORS` memberi sebaran. Hasil: 677 selesai -> SPI 0.67-1.5 (277 >1, 237 <1). | Untuk metrik "selesai", jangan ukur EV/PV di hari-ini (selalu 1). Ukur rencana-vs-aktual. Dokumentasikan divergensi dari rumus naskah di CLAUDE.md. |
| 2026-06-19 | FIX | 15 | Seed teknisi: 48 orang (terlalu banyak, sebagian wanita, nama depan duplikat dari random First+Last). Akar = volume 900 proyek/thn (peak 94 paralel). Fix: skala intake 3-6/hari + hari kosong, NUM_PROJECTS 750, CAPACITY 9 -> 23 teknisi; nama teknisi dikurasi (pria, depan UNIK) di `tech_names.csv`; nama klien perorangan dedup depan via `client_persons.csv`. | Headcount seed = gejala volume proyek (concurrency-bound, bukan count). Untuk SME realistis tuning intake x durasi x team, bukan tambah orang. Nama selalu cek duplikasi depan. |
| 2026-06-19 | FIX | 13 | Seed kebanyakan proyek `on-hold` (31): tak cocok citra perusahaan yang "tidak menunda pekerjaan". Hapus on-hold dari generator -> lewat-jadwal=completed, berjalan=active (677/73/0). Efek samping: membuang `rng.random()` di blok status menggeser RNG stream -> headcount berubah (22->retune CAPACITY 9 -> 23). | Status seed = cermin kebijakan bisnis klien, bukan sekadar sebaran acak. Mengubah jumlah draw `rng` menggeser seluruh stream deterministik -> WAJIB cek ulang invariant (headcount, sebaran SPI) tiap kali. |
