...
export default function TechnicianProjectsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit" role="tablist" aria-label="Project tabs">
        <button
          role="tab"
        ...
        <>
          {data.assigned_projects.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="mt-4 text-sm text-gray-500">No active projects assigned to you.</p>
        ...
        <>
          {completedProjects.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="mt-4 text-sm text-gray-500">No completed projects yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {completedProjects.map((proj) => (
                <button
                  key={proj.id}
                  onClick={() => navigate(`/projects/${proj.id}`)}
                  className="text-left bg-white hover:bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 p-5 transition-all opacity-75 hover:opacity-100"
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <h3 className="text-base font-semibold text-gray-600">{proj.name}</h3>
                    <div className="flex items-center gap-2 shrink-0">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 mb-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-600">{proj.client_name ?? 'No client assigned'}</p>
                        {proj.client_address && (
                          <p className="text-xs text-gray-400 mt-0.5">{proj.client_address}</p>
                        )}
                      </div>
