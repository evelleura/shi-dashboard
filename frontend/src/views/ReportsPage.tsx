...
type SummarySortKey = 'code' | 'name' | 'client' | 'status' | 'phase' | 'health' | 'spi' | 'tasks' | 'timeline' | 'value';

const SUMMARY_COLUMNS: { label: string; key: SummarySortKey | null; align?: 'right' }[] = [
  { label: 'Code', key: 'code' },
...
// Nilai banding per kolom utk sort tabel ringkasan. Health: proyek aktif diurut
// RAG (merah<amber<hijau<belum-dinilai), non-aktif (netral) ditaruh paling akhir
// supaya konsisten dgn aturan "RAG hanya proyek aktif".
function summarySortValue(p: DashboardProject, key: SummarySortKey): string | number {
  switch (key) {
    case 'code': return (p.project_code ?? '').toLowerCase();
    case 'name': return (p.name ?? '').toLowerCase();
    case 'client': return (p.client_name ?? '').toLowerCase();
    ...
    case 'spi': return p.spi_value != null ? Number(p.spi_value) : -Infinity;
    case 'tasks': return p.total_tasks > 0 ? p.completed_tasks / p.total_tasks : -1;
    case 'timeline': return new Date(p.start_date).getTime();
    case 'value': return Number(p.project_value) || 0;
    default: return 0;
  }
}
...
export default function ReportsPage() {
  ...
  // Sort tabel ringkasan. Default = kronologis terbaru dulu (timeline desc),
  // sama dgn perilaku sebelumnya; user bisa klik header utk urut kolom lain.
  const [sortKey, setSortKey] = useState<SummarySortKey>('timeline');
  ...
  // Tabel ringkasan: urutkan salinan sesuai kolom yg dipilih user (default timeline desc).
                ...
                <thead className="bg-gray-50 dark:bg-gray-900">
                  <tr>
                    {SUMMARY_COLUMNS.map((col) => (
                      <th key={col.label || 'actions'} className={`px-3 py-2 font-medium text-gray-500 dark:text-gray-400 uppercase ${col.align === 'right' ? 'text-right' : 'text-left'}`}>
                        {col.key ? (
                          <button
                            type="button"
                            onClick={() => handleSort(col.key as SummarySortKey)}
                            className={`inline-flex items-center gap-1 uppercase hover:text-gray-700 dark:hover:text-gray-200 transition-colors ${col.align === 'right' ? 'flex-row-reverse' : ''} ${sortKey === col.key ? 'text-gray-700 dark:text-gray-200' : ''}`}
                            aria-label={`Urutkan berdasarkan ${col.label}`}
                          >
                            {col.label}
                            <svg
                              className={`w-3 h-3 transition-all ${sortKey === col.key ? 'opacity-100' : 'opacity-30'} ${sortKey === col.key && sortDir === 'asc' ? 'rotate-180' : ''}`}
