  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Projects assigned to you with client details</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-700 p-1 rounded-lg w-fit" role="tablist" aria-label="Project tabs">
        <button
          role="tab"
        ...
        >
          Active ({data.assigned_projects.length})
        </button>
        <button
          role="tab"
        ...
        <>
          {data.assigned_projects.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
              ...
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">No active projects assigned to you.</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Ask your manager to assign you to a project.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 mb-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{proj.client_name ?? 'No client assigned'}</p>
                        {proj.client_address && (
        ...
        <>
          {completedProjects.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
              ...
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">No completed projects yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 mb-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{proj.client_name ?? 'No client assigned'}</p>
                        {proj.client_address && (
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{proj.client_address}</p>
                        )}
                      </div>
                  ...
                  {/* Final task count */}
                  <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
                    <span>Tasks: {proj.my_completed}/{proj.my_task_count} completed</span>
                    <span className="font-medium">
                      {proj.my_task_count > 0 ? Math.round((proj.my_completed / proj.my_task_count) * 100) : 0}%
                    </span>
                  </div>
