...
beforeEach(async () => {
  ...
  const mgr = await testPool.query(
    ...
    ['Test Manager', 'mgr@test.com', 'manajer', hash]
  );
  managerId = mgr.rows[0].id;

  const tech = await testPool.query(
    `INSERT INTO tb_user (nama, email, role, password) VALUES ($1,$2,$3,$4) RETURNING id_user AS id`,
    ['Test Tech', 'tech@test.com', 'teknisi', hash]
  );
  technicianId = tech.rows[0].id;

  const adm = await testPool.query(
    `INSERT INTO tb_user (nama, email, role, password) VALUES ($1,$2,$3,$4) RETURNING id_user AS id`,
    ['Test Admin', 'admin@test.com', 'manajer', hash]
  );
  ...
  const task = await testPool.query(
    `INSERT INTO tb_tugas (id_proyek, nama_tugas, description, assigned_to, status, due_date, sort_order, is_survey_task, created_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id_tugas AS id`,
...
describe('Tasks Handler', () => {
  describe('createTask', () => {
    it('returns 201 with created task for manager', async () => {
      const token = makeToken(managerId, 'manajer', 'mgr@test.com');
    ...
    it('returns 403 for technician trying to create task', async () => {
      const token = makeToken(technicianId, 'teknisi', 'tech@test.com');
      const req = makeRequest('POST', '/api/tasks', {
        project_id: projectId,
        name: 'New Task',
      }, token);
  ...
  describe('getTask', () => {
    ...
    it('returns 403 for unassigned technician', async () => {
      ...
      const otherTech = await testPool.query(
        `INSERT INTO tb_user (nama, email, role, password) VALUES ($1,$2,$3,$4) RETURNING id_user AS id`,
        ['Other Tech', 'other@test.com', 'teknisi', hash]
      );
      const token = makeToken(otherTech.rows[0].id, 'teknisi', 'other@test.com');
      const req = makeRequest('GET', `/api/tasks/${taskId}`, undefined, token);
      const res = await tasksHandler.getTask(req, String(taskId));
      expect(res.status).toBe(403);
    });
  ...
  describe('changeTaskStatus', () => {
    it('technician can change own task from to_do to in_progress', async () => {
      const token = makeToken(technicianId, 'teknisi', 'tech@test.com');
      const req = makeRequest('PATCH', `/api/tasks/${taskId}/status`, { status: 'working_on_it' }, token);
      const res = await tasksHandler.changeTaskStatus(req, String(taskId));
      const body = await res.json();
      expect(res.status).toBe(200);
      expect(body.data.status).toBe('working_on_it');
    });
    ...
    it('technician cannot change status of task not assigned to them', async () => {
      ...
      const otherTech = await testPool.query(
        `INSERT INTO tb_user (nama, email, role, password) VALUES ($1,$2,$3,$4) RETURNING id_user AS id`,
        ['Other Tech2', 'other2@test.com', 'teknisi', hash]
      );
      const token = makeToken(otherTech.rows[0].id, 'teknisi', 'other2@test.com');
      const req = makeRequest('PATCH', `/api/tasks/${taskId}/status`, { status: 'working_on_it' }, token);
      const res = await tasksHandler.changeTaskStatus(req, String(taskId));
      expect(res.status).toBe(403);
    });
    ...
    it('accumulates time_spent_seconds when leaving an active status', async () => {
      ...
      const token = makeToken(technicianId, 'teknisi', 'tech@test.com');
      let req = makeRequest('PATCH', `/api/tasks/${taskId}/status`, { status: 'working_on_it' }, token);
      ...
      await testPool.query(
        `UPDATE tb_tugas SET status_changed_at = NOW() - INTERVAL '120 seconds' WHERE id_tugas=$1`,
        [taskId]
      );

      req = makeRequest('PATCH', `/api/tasks/${taskId}/status`, { status: 'to_do' }, token);
      const res = await tasksHandler.changeTaskStatus(req, String(taskId));
      const body = await res.json();
      expect(res.status).toBe(200);
      expect(body.data.status).toBe('to_do');
    ...
    it('returns 404 for non-existent task', async () => {
      const token = makeToken(managerId, 'manajer', 'mgr@test.com');
      const req = makeRequest('PATCH', '/api/tasks/99999/status', { status: 'working_on_it' }, token);
    ...
    it('returns 400 for invalid task ID', async () => {
      const token = makeToken(managerId, 'manajer', 'mgr@test.com');
      const req = makeRequest('PATCH', '/api/tasks/abc/status', { status: 'working_on_it' }, token);
    ...
    it('blocks status change if dependent task is not done', async () => {
      ...
      const depTask = await testPool.query(
        `INSERT INTO tb_tugas (id_proyek, nama_tugas, status, due_date, sort_order, is_survey_task, created_by)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id_tugas AS id`,
      ...
      await testPool.query(`UPDATE tb_tugas SET depends_on=$1 WHERE id_tugas=$2`, [depTaskId, taskId]);
    ...
    it('blocks non-survey task when project is in survey phase', async () => {
      await testPool.query(`UPDATE tb_proyek SET phase='survey' WHERE id_proyek=$1`, [projectId]);
      const token = makeToken(technicianId, 'teknisi', 'tech@test.com');
      const req = makeRequest('PATCH', `/api/tasks/${taskId}/status`, { status: 'working_on_it' }, token);
      const res = await tasksHandler.changeTaskStatus(req, String(taskId));
      expect(res.status).toBe(400);
