...
beforeEach(async () => {
  await testPool.query(`TRUNCATE TABLE users, audit_log RESTART IDENTITY CASCADE`);

  const hash = await bcrypt.hash('Password123!', 10);
  const mgr = await testPool.query(
    `INSERT INTO users (name, email, role, password_hash, is_active) VALUES ($1,$2,$3,$4,TRUE) RETURNING id`,
...
describe('Users Handler', () => {
  ...
  describe('createUser', () => {
    ...
    it('returns 403 for manager', async () => {
      const token = makeToken(managerId, 'manager', 'mgr@t.com');
      const req = makeRequest('POST', '/api/users', {
        name: 'X', email: 'x@t.com', password: 'pw', role: 'technician',
      }, token);
      const res = await usersHandler.createUser(req);
      expect(res.status).toBe(403);
    });
    ...
    it('returns 400 when password too short', async () => {
      const token = makeToken(adminId, 'admin', 'admin@t.com');
      const req = makeRequest('POST', '/api/users', {
        name: 'X', email: 'x2@t.com', password: 'abc', role: 'technician',
      }, token);
      const res = await usersHandler.createUser(req);
      expect(res.status).toBe(400);
    });
    ...
    it('returns 409 on duplicate email', async () => {
      const token = makeToken(adminId, 'admin', 'admin@t.com');
      const req = makeRequest('POST', '/api/users', {
        name: 'Dup', email: 'mgr@t.com', password: 'Password123!', role: 'manager',
      }, token);
      const res = await usersHandler.createUser(req);
  ...
  describe('deactivateUser / activateUser', () => {
    ...
    it('activates user for admin', async () => {
      // First deactivate
      await testPool.query(`UPDATE users SET is_active=FALSE WHERE id=$1`, [technicianId]);
      const token = makeToken(adminId, 'admin', 'admin@t.com');
    ...
    it('returns 400 when deactivating last active admin', async () => {
      ...
      await testPool.query(`UPDATE users SET is_active=FALSE WHERE role='admin' AND id!=$1`, [adminId]);
      const token = makeToken(adminId, 'admin', 'admin@t.com');
      ...
      const token2 = makeToken(admin2.rows[0].id, 'admin', 'admin2@t.com');
