...
export async function uploadEvidence(request: NextRequest) {
  ...
  try {
    const taskCheck = await query('SELECT id_tugas AS id, id_proyek AS project_id FROM tb_tugas WHERE id_tugas = $1', [taskId]);
    ...
    const result = await query(
      `INSERT INTO tb_bukti (id_tugas, file_path, file_name, file_type, file_size, description, uploaded_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id_bukti AS id, id_tugas AS task_id, file_path, file_name, file_type, file_size, description, uploaded_by, uploaded_at`,
...
export async function deleteEvidence(request: NextRequest, id: string) {
  ...
  try {
    const result = await query('SELECT file_path FROM tb_bukti WHERE id_bukti = $1', [evidenceId]);
    ...
    await query('DELETE FROM tb_bukti WHERE id_bukti = $1', [evidenceId]);
...
export async function downloadEvidence(request: NextRequest, id: string) {
  ...
  try {
    const result = await query('SELECT file_path, file_name FROM tb_bukti WHERE id_bukti = $1', [evidenceId]);
    if (result.rowCount === 0) return NextResponse.json({ success: false, error: 'Evidence not found' }, { status: 404 });
