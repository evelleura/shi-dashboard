...
const AGE_COLORS: Record<RowAge, string> = {
  new:    'bg-blue-50/60 dark:bg-blue-900/20 border-l-4 border-l-blue-400',
  recent: 'bg-emerald-50/40 dark:bg-emerald-900/20 border-l-4 border-l-emerald-300 dark:border-l-emerald-600',
  normal: '',
  stale:  'bg-gray-50/60 dark:bg-gray-800/60 border-l-4 border-l-gray-300 dark:border-l-gray-600 text-gray-400 dark:text-gray-500',
};
              ...
              {visibleColumns.map((col) => {
                ...
                return (
                  <th
                    ...
                    } ${isSortable ? 'cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 select-none' : ''} ${col.headerClass ?? ''}`}
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((row, idx) => {
                ...
                return (
                  <tr
                    key={rowKey(row)}
                    className={`${ageClass} ${extra} ${onRowClick ? 'cursor-pointer' : ''} hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-colors`}
