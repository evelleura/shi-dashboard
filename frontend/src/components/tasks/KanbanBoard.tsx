...
// Naskah 3-status (to_do/working_on_it/done). Kanban menampilkan 5 kolom visual:
// 3 status nyata + 2 kolom TERHITUNG (lembur, lewat tenggat) dari perbandingan tanggal.
...
function getColumns(language: 'id' | 'en'): ColumnDef[] {
  return [
    ...
    { id: 'working_on_it', label: t('status.working_on_it', language), borderColor: 'border-t-blue-500', headerBg: 'bg-blue-50 dark:bg-blue-900/30', headerText: 'text-blue-700 dark:text-blue-400', dropBg: 'bg-blue-50/50 dark:bg-blue-900/10' },
...
function getTaskColumn(task: Task): KanbanColumn {
  ...
  if (task.status === 'working_on_it' && isOverdue(task)) return 'overtime';
  if (task.status === 'to_do' && isOverdue(task)) return 'over_deadline';
  if (task.status === 'working_on_it') return 'working_on_it';
  return 'to_do';
}
...
export default function KanbanBoard({ tasks, onStatusChange, onTaskClick, changingTaskId, userRole }: Props) {
  ...
  const grouped = useMemo(() => {
    const map: Record<KanbanColumn, Task[]> = { to_do: [], working_on_it: [], done: [], overtime: [], over_deadline: [] };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // dragging from over_deadline -> to_do, or overtime -> working_on_it).
    if (newStatus === draggedTask.status) return;

    // Review gate: teknisi tidak bisa menandai 'done' (hanya manajer).
    if (isTechnician && newStatus === 'done') return;
